...
resource "aws_security_group" "ec2-active-directory-op-aws" {
  name        = var.security_group_to-active-directory
  description = "ec2-to-active-directory"
  vpc_id      = aws_vpc.main.id

  dynamic "ingress" {
    for_each = var.ingress_rules-to-active-directory
    content {
      from_port   = ingress.value.from_port
      to_port     = ingress.value.to_port
      protocol    = ingress.value.protocol
  ...
  dynamic "egress" {
    for_each = var.egress_rules-to-active-directory
    content {
      from_port   = egress.value.from_port
      to_port     = egress.value.to_port
      protocol    = egress.value.protocol
